# -*- coding: utf-8 -*-
"""
Created on Fri Nov 17 20:35:33 2017
...
class Data():
    def __init__(self, symbol_universe=None):
        ...
        if symbol_universe is None:
            ...
            # Top N by market cap
            symbol_list = symbols_combined.sort_values('MarketCap').Symbol.tail(550).values
    ...
    def get_historical(self, symbol):
        ...
        try:
            ...
            f = web.DataReader(symbol.strip(), 'google', self.start, self.end).dropna(axis=0)
 
            if len(f.index) < self.hist_len*0.95: #required domain check
                self.symbols.remove(symbol)
                #print 1, symbol
                print("Failed to download enough history for: %s"%symbol, len(f.index), self.hist_len)
                return None, None
            return symbol, f
        except:
