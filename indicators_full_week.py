# -*- coding: utf-8 -*-
"""
Created on Sat Dec 16 21:10:56 2017
...
def get_weeks(df):
    ...
    for index,day in enumerate(reversed(df.index)):
        if day.weekday() == 0:
            Weeklist.append([df.index[index:index+5]])
    return Weeklist
...
def get_last_100_days(symbol,retry_count):
    ...
    try:
        ...
        smalldf = df.head(500)
        Weeklist = get_weeks(smalldf)
        
        return smalldf,Weeklist
    except Exception:
        retry_count +=1
...
class ranking():
    ...
    def __init__(self,symbol,date,bp,sp,stoch,ma,rsi):
        ...
        self.p1 = self.SellPrice[0]
        self.p2 = self.SellPrice[1]
        self.p3 = self.SellPrice[2]
        self.p4 = self.SellPrice[3]
...
for symbol in symbols:
    try:
        ...
        print()
        df,Weeklist = get_last_100_days(symbol,retry_count)
        ...
        for week in Weeklist:
            print(week[0])
            sp = []
            mondaydate = week[0].strftime('%Y-%m-%d')
            ticker = symbol
            date   = mondaydate
            bp = df.get_value(week[0],'open')
            print(week)
            for day in week[1:]:
                print(day)
                dt = df.get_value(day,'close')
                sp.append(dt)
            st     = [stoch.get_value(mondaydate,'SlowK'),stoch.get_value(mondaydate,'SlowD')]
            m      = [ma.get_value(mondaydate,'15EMA'),ma.get_value(mondaydate,'60EMA')]
            r      = rsi.get_value(mondaydate,'RSI')
        ...
        print()
        with open('full_week//output.txt','a') as f:        
            for obj in objectlist:
                f.write(str(obj)+'\n')
            f.close()
            objectlist.clear()
    except Exception:
        print('Error with week for %s'%symbol,week[0],bp,sp,st,m,r)
        print()
        continue
